...
3. **Schema Migration**: 코드 배포 후 Migration 실행이 필요한 경우, Apps Script 편집기에서 수동 실행 (`runMigrations()`는 `SpreadsheetApp.getUi()`를 쓰므로 CI에서 실행할 수 없다)
   - **열 구조를 바꾸는 마이그레이션은 배포 직후 곧바로 실행한다.** 코드는 push로 즉시 나가지만
     마이그레이션은 사람이 눌러야 돈다. 그 사이 창에서 코드가 믿는 열 번호와 시트의 실제 열이
     어긋나고, 그 상태에서 값을 되쓰는 함수(재고 재계산, 품목 정렬)가 한 번이라도 돌면
     계산 결과가 옆 열의 운영 데이터를 덮어쓴다. DEV에서 실제로 일어났다 —
     v17 배포 후 마이그레이션 전에 재고 합계금액이 97개 품목의 사용유무를 통째로 지웠다(v18에서 복구).
   - 지금은 `_isMasterSchemaCurrent()` 가드가 이 창의 쓰기를 막지만, 가드는 **쓰기를 건너뛸 뿐**이라
     그 사이 재고/평가액은 갱신되지 않는다. 창을 짧게 유지하는 것이 여전히 맞다.
4. **배포를 새로 만들지 말 것**: 확인용이라도 `clasp deploy`를 `-i` 없이 실행하면 새 URL이 생긴다. 환경당 배포는 1개로 유지한다
5. **TASK-019 배포 후 수동 절차** (DEV → Production 각각, 스프레드시트를 연 상태에서):
   1. 시트 메뉴 `🏨 관리자 도구 > 🎨 시트 서식/검증 복구` 1회 실행 — 품목 마스터 거래처코드(S열)의 엄격 검증(목록 외 입력 차단)을
      전체 행에 재적용한다. 코드는 이미 `setAllowInvalid(false)`이지만 서식은 적용 시점의 스냅샷이라 기존 시트에는 이 실행이 필요하다.
   2. Apps Script 편집기에서 `migrateShops_TASK019()` 1회 실행 — 기존 업장 시트를 **영구 삭제**하고 23개 업장을 새로 만든다.
      `runMigrations()`에 포함되지 않는다(스키마 버전과 무관한 운영 작업). 실행 전 `파일 > 사본 만들기`로 스냅샷을 확보할 것.
      CSV 백업 → 업장관리 갱신 → 시트 삭제 → 시트 생성 → 통합 갱신 순으로 돌고, YES/NO 확인창이 스프레드시트에 뜬다.
   3. 👤 사용자관리 F열(배정 업장)이 옛 업장명을 가리키는 계정은 웹앱 `계정 관리`에서 다시 배정한다.
   - 관리자 도구의 모든 항목은 이제 실행 전 안내문 + YES/NO 확인창을 거친다. 트리거·웹앱이 부르는 프로그램 경로(`refreshDashboard(true)` 등)는 그대로다.
