...
- 서버 API가 돌려주는 결과 문구 앞의 이모지(`✅`·`❌` 등 예전 메시지)는 화면에 그대로 내지 않는다 — 토스트·모달에 넣기 전에 떼어 낸다 (`JS_Items itemMsg`, `AdminActionDialog.html stripLeadingEmoji`).
...
- **대시보드 헤더 시각** (TASK-029): `재고 계산 기준: yyyy-MM-dd HH:mm` — 서버가 스크립트 타임존으로 포맷한 재고 재계산 시각(`recalcAtText`)만 쓴다. 조회·캐시 시각을 "최신 갱신일"로 보이게 하지 않는다. 기록이 없으면 `재고 계산 기준: 기록 없음`.
- **오래된 계산 배지**: 재계산이 24시간보다 오래됐을 때만 헤더 시각 옆에 `.status-badge order`(앰버) pill `하루 이상 지난 계산`(`#dashStaleBadge`, 기본 `hidden`, 사유는 `title`). 상시 노출·이모지·별도 색 금지 — 3절·5절 규칙 그대로.
